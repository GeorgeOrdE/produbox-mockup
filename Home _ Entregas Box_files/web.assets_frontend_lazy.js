...
odoo.define('website.root', function (require) {
    'use strict'; const ajax = require('web.ajax'); const { _t } = require('web.core'); var Dialog = require('web.Dialog'); const KeyboardNavigationMixin = require('web.KeyboardNavigationMixin'); const session = require('web.session'); var publicRootData = require('web.public.root'); require("web.zoomodoo"); var websiteRootRegistry = publicRootData.publicRootRegistry; var WebsiteRoot = publicRootData.PublicRoot.extend(KeyboardNavigationMixin, {
        events: _.extend({}, KeyboardNavigationMixin.events, publicRootData.PublicRoot.prototype.events || {}, { 'click .js_change_lang': '_onLangChangeClick', 'click .js_publish_management .js_publish_btn': '_onPublishBtnClick', 'click .js_multi_website_switch': '_onWebsiteSwitch', 'shown.bs.modal': '_onModalShown', }), custom_events: _.extend({}, publicRootData.PublicRoot.prototype.custom_events || {}, { 'gmap_api_request': '_onGMapAPIRequest', 'gmap_api_key_request': '_onGMapAPIKeyRequest', 'ready_to_clean_for_save': '_onWidgetsStopRequest', 'seo_object_request': '_onSeoObjectRequest', 'will_remove_snippet': '_onWidgetsStopRequest', }), init() { this.isFullscreen = false; KeyboardNavigationMixin.init.call(this, { autoAccessKeys: false, }); return this._super(...arguments); }, start: function () {
    ...
    publicWidget.Widget.include({ disabledInEditableMode: true, edit_events: null, read_events: null, init: function (parent, options) { this._super.apply(this, arguments); this.editableMode = this.options.editableMode || false; var extraEvents = this.editableMode ? this.edit_events : this.read_events; if (extraEvents) { this.events = _.extend({}, this.events || {}, extraEvents); } }, }); var AnimationEffect = Class.extend(mixins.ParentedMixin, {
        ...
        maxFPS: 100, effects: [], start: function () { this._prepareEffects(); _.each(this._animationEffects, function (effect) { effect.start(); }); return this._super.apply(this, arguments); }, _prepareEffects: function () {
            this._animationEffects = []; var self = this; _.each(this.effects, function (desc) {
                self._addEffect(self[desc.update], desc.startEvents, _findTarget(desc.startTarget), { getStateCallback: desc.getState && self[desc.getState], endEvents: desc.endEvents || undefined, $endTarget: _findTarget(desc.endTarget), maxFPS: self.maxFPS, }); function _findTarget(selector) {
                    if (selector) {
        ...
        selector: '.parallax', disabledInEditableMode: false, effects: [{ startEvents: 'scroll', update: '_onWindowScroll', }], start: function () { this._rebuild(); $(window).on('resize.animation_parallax', _.debounce(this._rebuild.bind(this), 500)); return this._super.apply(this, arguments); }, destroy: function () { this._super.apply(this, arguments); $(window).off('.animation_parallax'); }, _rebuild: function () {
...
/* /payment_datafast/static/src/js/datafast_refund.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('payment_datafast.datafast_refund', function (require) {
    var ajax = require('web.ajax'); var Dialog = require('web.Dialog'); var core = require('web.core'); var _t = core._t; if ($.blockUI) { $.blockUI.defaults.css.border = '0'; $.blockUI.defaults.css["background-color"] = ''; $.blockUI.defaults.overlayCSS["opacity"] = '0.9'; }
    $(document).ready(function () {
        var datafastRefundModal = $('#datafastRefundModal'); if (!datafastRefundModal)
            return; var $form = $('form.datafast_refund'); var $button = $('#submit_datafast_refund'); function disable_button() { $button.attr("disabled", "disabled"); }
        function enable_button() { $button.removeAttr("disabled"); }
        $form.submit(function (ev) {
            ev.preventDefault(); var card_number = $form.find('input[name=cc_number]').val().trim(); var valid_card_value = $.payment.validateCardNumber(card_number); var card_type = $.payment.cardType(card_number); if (card_type) { $(this).parent('.form-group').children('.card_placeholder').removeClass().addClass('card_placeholder ' + card_type); $(this).parent('.form-group').children('input[name="cc_brand"]').val(card_type); }
            else { $(this).parent('.form-group').children('.card_placeholder').removeClass().addClass('card_placeholder'); }
            if (valid_card_value) { $(this).parent('.form-group').addClass('has-success'); $(this).parent('.form-group').removeClass('has-error'); $(this).siblings('.o_invalid_field').remove(); }
            else { $(this).parent('.form-group').addClass('has-error'); $(this).parent('.form-group').removeClass('has-success'); return; }
            var expiry = $form.find('input[name=cc_expiry]').val(); var expiry_value = $.payment.cardExpiryVal(expiry); var month = expiry_value.month || ''; var year = expiry_value.year || ''; var valid_expiry_value = $.payment.validateCardExpiry(month, year); if (valid_expiry_value) { $form.find('input[name=cc_expiry]').parent('.form-group').addClass('has-success'); $form.find('input[name=cc_expiry]').parent('.form-group').removeClass('has-error'); $form.find('input[name=cc_expiry]').siblings('.o_invalid_field').remove(); }
            else { $form.find('input[name=cc_expiry]').parent('.form-group').addClass('has-error'); $form.find('input[name=cc_expiry]').parent('.form-group').removeClass('has-success'); return; }
            var tx_id = $form.find('input[name=tx_id]').val(); var msg = _t("Just one more second, confirming your refund..."); $.blockUI({ 'message': '<h2 class="text-white"><img src="/web/static/src/img/spin.png" class="fa-pulse"/>' + '    <br />' + msg + '</h2>' }); disable_button(); ajax.jsonRpc("/payment/datafast/create_refund", 'call', { 'card_number': card_number, 'expiry_month': month, 'expiry_year': year, 'tx_id': tx_id, }).then(function (data) {
                if (data.error) { enable_button() }
                Dialog.alert($('body'), data.message, { confirm_callback: function () { window.location.reload(); }, title: _t('Response Process Refund'), }); $form.find('input[name=cc_number]').val(""); $form.find('input[name=cc_expiry]').val(""); datafastRefundModal.modal('hide');
            }).fail(function (data) { var message = _t("Code:" + data.code + "\n" + "Message:" + data.message); Dialog.alert($('body'), message, { title: _t('Error') }); enable_button() }).always(function () { $.unblockUI(); }); return true;
        }); datafastRefundModal.on('shown.bs.modal', function () { $('#cc_number').trigger('focus') }); datafastRefundModal.on('hidden.bs.modal', function (e) { $form.find('input[name=cc_number]').val(""); $form.find('input[name=cc_expiry]').val(""); });
    });
});;

/* /website_e_box/static/src/js/website.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('website_e_box.package', function (require) {
    'use strict'; var ajax = require('web.ajax'); var publicWidget = require('web.public.widget'); var core = require('web.core'); var _t = core._t; var Dialog = require('web.Dialog'); publicWidget.registry.websitePackageList = publicWidget.Widget.extend({
        selector: '.package-validation', events: { 'click a.a-submit-delivery': '_onClickDeliveryJson', }, _onClickDeliveryJson: function (ev) {
            var self = this; var package_ids = []; var $Inputs = this.$('input[type=checkbox]:checked'); var totalPrice = 0.0; var totalWeight = 0.0; $Inputs.each(function () { var $tr = $(this).closest('tr'); package_ids.push(parseInt($(this).prop("id"))); var $SpanTotal = $tr.find("input[name=total_amount]"); var $SpanWeight = $tr.find("input[name=package_weight]"); totalPrice = totalPrice + parseFloat($SpanTotal.val() || 0); totalWeight = totalWeight + parseFloat($SpanWeight.val() || 0); }); if ($Inputs.length === 0) { Dialog.alert(this, "Disculpe, debe seleccionar al menos un paquete!!!"); return; }
            if (totalPrice <= 400 && totalWeight <= 4) {
                ajax.jsonRpc('/view/modal/package', 'call', { 'package_ids': package_ids, 'user_type': '4x4' }).then(function (data) {
                    if (data.error === true) { Dialog.alert(self, data.message); return; }
                    var $modal = $(data['view']); $modal.modal({ backdrop: 'static', keyboard: false }); $modal.find('.modal-body > div').removeClass('container'); $modal.appendTo('body').modal(); $modal.on('click', '.js_cancel_event', function () { $modal.modal('hide'); }); $modal.on('click', '.btn-footer-modal', function () {
                        var $selectAddress = $modal.find('select[name=contact_id]'); var $inputInsurance = $modal.find('input[type=checkbox]:checked'); var insurance = $inputInsurance.val() || 0
                        self._rpc({ route: '/json/create/delivery', params: { 'package_ids': package_ids, 'address_id': parseInt($selectAddress.val()), 'insurance_carrier': parseInt(insurance) } }).then(function (response) {
                            if (response.success === false) { Dialog.alert(self, response.message); return; }
                            window.location = response.return_url;
                        }); return;
                    });
                });
            } else {
                ajax.jsonRpc('/view/modal/package', 'call', { 'package_ids': package_ids, 'user_type': 'cxc' }).then(function (data) {
                    var $modal = $(data['view']); $modal.modal({ backdrop: 'static', keyboard: false }); $modal.find('.modal-body > div').removeClass('container'); $modal.appendTo('body').modal(); $modal.on('click', '.js_cancel_event', function () { $modal.modal('hide'); }); $modal.on('click', '.btn-footer-modal', function () {
                        var $selectAddress = $modal.find('select[name=contact_id]'); var $inputInsurance = $modal.find('input[type=checkbox]:checked'); var insurance = $inputInsurance.val() || 0; var $tr = $modal.find('tr[name=product_line]'); var tax_line = []; _.each($tr, function (current_tr) { var param = {}; _.each($(current_tr).find('td'), function (current_td) { var $input = $(current_td).find('input'); if ($input.length == 1) { param[$input.attr('name')] = $input.val() || '0'; } }); tax_line.push(param); }); self._rpc({ route: '/json/create/shipping', params: { 'package_ids': package_ids, 'address_id': parseInt($selectAddress.val()), 'insurance_carrier': parseInt(insurance), 'tax_line': tax_line } }).then(function (response) {
                            if (response.success === false) { Dialog.alert(self, response.message); return; }
                            window.location = response.return_url;
                        }); return;
                    });
    ...
    }); publicWidget.registry.PreAlertForm = publicWidget.Widget.extend({
        selector: '.oe_form_prealert', events: { 'click button[type="submit"]': '_onSubmitValidate', }, _onSubmitValidate: function (e) {
            var values = {}; e.preventDefault(); var $btn = $(e.currentTarget); var $div_tracking = this.$('div.missing-tracking'); var $div_description = this.$('div.missing-description'); $btn.attr('disabled', 'disabled'); $btn.prepend('<i class="fa fa-refresh fa-spin"/> '); var $InputNumber = this.$('input[name=package_number]'); var $textArea = this.$('textarea[name=package_description]'); values['package_number'] = $InputNumber.val() || false; values['package_description'] = $textArea.val() || false; if (!$div_tracking.hasClass('d-none')) { $div_tracking.addClass('d-none'); }
            if (!$div_description.hasClass('d-none')) { $div_description.addClass('d-none'); }
            if (values.package_number === false) { $div_tracking.removeClass('d-none'); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
            if (values.package_description === false) { $div_description.removeClass('d-none'); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
            ajax.jsonRpc('/my/locker/prealert/submit', 'call', values).then(function (data) {
                if (data.success === false) {
                    var message = data.message || "Error procesando la pre alerta."
                    Dialog.alert(this, message); $btn.removeAttr('disabled'); $btn.find('i').remove(); return;
                }
                window.location.reload(); $InputNumber.val(""); $textArea.val(""); return;
            });
        },
    }); publicWidget.registry.PreAlertUpdate = publicWidget.Widget.extend({
        selector: '.oe_form_prealert_edit', events: { 'click button[type="submit"]': '_onSubmitUpdate', }, _onSubmitUpdate: function (e) {
            var values = {}; e.preventDefault(); var $btn = $(e.currentTarget); var $div_tracking = this.$('div.missing-tracking'); var $div_description = this.$('div.missing-description'); $btn.attr('disabled', 'disabled'); $btn.prepend('<i class="fa fa-refresh fa-spin"/> '); var $InputNumber = this.$('input[name=package_number]'); var $textArea = this.$('textarea[name=package_description]'); var $InputId = this.$('input[name=package_id]'); values['package_number'] = $InputNumber.val() || false; values['package_description'] = $textArea.val() || false; values['package_id'] = $InputId.val() || false; if (!$div_tracking.hasClass('d-none')) { $div_tracking.addClass('d-none'); }
            if (!$div_description.hasClass('d-none')) { $div_description.addClass('d-none'); }
            if (values.package_number === false) { $div_tracking.removeClass('d-none'); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
            if (values.package_description === false) { $div_description.removeClass('d-none'); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
            ajax.jsonRpc('/my/locker/prealert/update', 'call', values).then(function (data) {
                if (data.success === false) {
                    var message = data.message || "Error procesando la pre alerta."
                    Dialog.alert(this, message); $btn.removeAttr('disabled'); $btn.find('i').remove(); return;
                }
                window.location.replace('/my/locker/prealert'); return;
            });
        },
    });
});;
...
odoo.define('mass_mailing.website_integration', function (require) {
    "use strict"; var config = require('web.config'); var core = require('web.core'); const dom = require('web.dom'); var Dialog = require('web.Dialog'); var utils = require('web.utils'); var publicWidget = require('web.public.widget'); const session = require('web.session'); var _t = core._t; let alertReCaptchaDisplayed; publicWidget.registry.subscribe = publicWidget.Widget.extend({
        selector: ".js_subscribe", disabledInEditableMode: false, read_events: { 'click .js_subscribe_btn': '_onSubscribeClick', }, init: function () { this._super(...arguments); const ReCaptchaService = odoo.__DEBUG__.services['google_recaptcha.ReCaptchaV3']; this._recaptcha = ReCaptchaService && new ReCaptchaService.ReCaptcha() || null; }, willStart: function () {
            ...
            this._rpc({ route: '/website_mass_mailing/subscribe', params: params, }).then(function (result) {
                let toastType = result.toast_type; if (toastType === 'success') { self.$(".js_subscribe_btn").addClass('d-none'); self.$(".js_subscribed_btn").removeClass('d-none'); self.$('input.js_subscribe_email').prop('disabled', !!result); if (self.$popup.length) { self.$popup.modal('hide'); } }
        ...
        selector: ".o_newsletter_popup", disabledInEditableMode: false, start: function () {
            var self = this; var defs = [this._super.apply(this, arguments)]; this.websiteID = this._getContext().website_id; this.listID = parseInt(this.$target.attr('data-list-id')); if (!this.listID || (utils.get_cookie(_.str.sprintf("newsletter-popup-%s-%s", this.listID, this.websiteID)) && !self.editableMode)) { return Promise.all(defs); }
            if (this.$target.data('content') && this.editableMode) { this._dialogInit(this.$target.data('content')); this.$target.removeData('quick-open'); this.massMailingPopup.open(); } else {
                defs.push(this._rpc({ route: '/website_mass_mailing/get_content', params: { newsletter_id: self.listID, }, }).then(function (data) {
                    self._dialogInit(data.popup_content, data.email || ''); if (!self.editableMode && !data.is_subscriber) { if (config.device.isMobile) { setTimeout(function () { self._showBanner(); }, 5000); } else { $(document).on('mouseleave.open_popup_event', self._showBanner.bind(self)); } } else { $(document).off('mouseleave.open_popup_event'); }
                    if (self.$target.data('quick-open')) { self.massMailingPopup.open(); self.$target.removeData('quick-open'); }
                }));
            }
            return Promise.all(defs);
        }, destroy: function () {
            if (this.massMailingPopup) { this.massMailingPopup.close(); }
            this._super.apply(this, arguments);
...
odoo.define('website.ripple_effect', function (require) {
    'use strict'; const publicWidget = require('web.public.widget'); publicWidget.registry.RippleEffect = publicWidget.Widget.extend({
        selector: '.btn, .dropdown-toggle, .dropdown-item', events: { 'click': '_onClick', }, duration: 350, start: async function () { this.diameter = Math.max(this.$el.outerWidth(), this.$el.outerHeight()); this.offsetX = this.$el.offset().left; this.offsetY = this.$el.offset().top; return this._super(...arguments); }, destroy: function () { this._super(...arguments); if (this.rippleEl) { this.rippleEl.remove(); } }, _toggleRippleEffect: function (toggle) { this.el.classList.toggle('o_js_ripple_effect', toggle); }, _onClick: function (ev) {
            if (!this.rippleEl) { this.rippleEl = document.createElement('span'); this.rippleEl.classList.add('o_ripple_item'); this.rippleEl.style.animationDuration = `${this.duration}ms`; this.el.appendChild(this.rippleEl); }
...
odoo.define('website_mail_channel', function (require) {
    'use strict'; var publicWidget = require('web.public.widget'); publicWidget.registry.websiteMailChannel = publicWidget.Widget.extend({
        selector: '#wrapwrap', events: { 'click .o_mg_link_hide': '_onHideLinkClick', 'click .o_mg_link_show': '_onShowLinkClick', 'click button.o_mg_read_more': '_onReadMoreClick', }, _onHideLinkClick: function (ev) { ev.preventDefault(); ev.stopPropagation(); var $link = $(ev.currentTarget); var $container = $link.parents('div').first(); $container.find('.o_mg_link_hide').first().hide(); $container.find('.o_mg_link_show').first().show(); $container.find('.o_mg_link_content').first().show(); }, _onShowLinkClick: function (ev) { ev.preventDefault(); ev.stopPropagation(); var $link = $(ev.currentTarget); var $container = $link.parents('div').first(); $container.find('.o_mg_link_hide').first().show(); $container.find('.o_mg_link_show').first().hide(); $container.find('.o_mg_link_content').first().hide(); }, _onReadMoreClick: function (ev) {
            var $link = $(ev.target); this._rpc({ route: $link.data('href'), params: { last_displayed_id: $link.data('msg-id'), }, }).then(function (data) {
                if (!data) { return; }
...
odoo.define('website_mail_channel.s_channel', function (require) {
    'use strict'; var publicWidget = require('web.public.widget'); publicWidget.registry.Channel = publicWidget.Widget.extend({
        selector: '.s_channel', disabledInEditableMode: false, read_events: { 'click .js_follow_btn, .js_unfollow_btn': '_onFollowUnFollowBtnClick', 'click .js_follow_btn': '_onFollowBtnClick', }, start: function () {
            var self = this; this.is_user = false; var unsubscribePage = window.location.search.slice(1).split('&').indexOf("unsubscribe") >= 0; var always = function (data) {
                self.is_user = data.is_user; self.email = data.email; self.$target.find('.js_mg_link').attr('href', '/groups/' + self.$target.data('id')); if (unsubscribePage && self.is_user) { self.$target.find(".js_mg_follow_form").remove(); }
                self._toggleSubscription(data.is_member ? 'on' : 'off', data.email); self.$target.removeClass('d-none');
            }; this._rpc({ route: '/groups/is_member', params: { model: this.$target.data('object'), channel_id: this.$target.data('id'), get_alias_info: true, }, }).then(always).guardedCatch(always); if (!this.editableMode) { this.$('> .alert').addClass('d-none'); this.$('> .input-group-append.d-none').removeClass('d-none'); }
            return this._super.apply(this, arguments);
        }, _getAliasInfo: function () {
            var self = this; if (!this.$target.data('id')) { return Promise.resolve(); }
            return this._rpc({ route: '/groups/' + this.$target.data('id') + '/get_alias_info' }).then(function (data) { if (data.alias_name) { self.$target.find('.js_mg_email').attr('href', 'mailto:' + data.alias_name); self.$target.find('.js_mg_email').removeClass('d-none'); } else { self.$target.find('.js_mg_email').addClass('d-none'); } });
        }, _toggleSubscription: function (follow, email) {
            var aliasDone = this._getAliasInfo(); if (follow === "on") { this.$target.find(".js_mg_follow_form").addClass('d-none'); this.$target.find(".js_mg_details").removeClass('d-none'); this.$target.find(".js_mg_confirmation").addClass('d-none'); } else if (follow === "off") { this.$target.find(".js_mg_follow_form").removeClass('d-none'); this.$target.find(".js_mg_details").addClass('d-none'); this.$target.find(".js_mg_confirmation").addClass('d-none'); } else if (follow === "email") { this.$target.find(".js_mg_follow_form").addClass('d-none'); this.$target.find(".js_mg_details").addClass('d-none'); this.$target.find(".js_mg_confirmation").removeClass('d-none'); } else { console.error("Unknown subscription action", follow); }
            this.$target.find('input.js_follow_email').val(email ? email : "").attr("disabled", follow === "on" || (email.length && this.is_user) ? "disabled" : false); this.$target.attr("data-follow", follow); return Promise.resolve(aliasDone);
        }, _onFollowBtnClick: function (ev) { if ($(ev.currentTarget).closest('.js_mg_follow_form').length) { this.$('.js_follow_email').val($(ev.currentTarget).closest('.js_mg_follow_form').find('.js_follow_email').val()); } }, _onFollowUnFollowBtnClick: function (ev) {
            ev.preventDefault(); var self = this; var $email = this.$target.find(".js_follow_email"); if ($email.length && !$email.val().match(/.+@.+/)) { this.$target.addClass('o_has_error').find('.form-control, .custom-select').addClass('is-invalid'); return false; }
...
/* /theme_e_box/static/src/js/ebox.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('theme_e_box.form_locker', function (require) {
    'use strict'; var Dialog = require('web.Dialog'); var publicWidget = require('web.public.widget'); var ajax = require('web.ajax'); publicWidget.registry.ButtonSubmitBlock = publicWidget.Widget.extend({ selector: ".button_submit_block", events: { 'submit': '_onSubmit', }, _onSubmit: function () { var $btn = this.$('button[type="submit"]'); $btn.attr('disabled', 'disabled'); $btn.prepend('<i class="fa fa-refresh fa-spin"/> '); }, }); publicWidget.registry.FormLockerCreate = publicWidget.Widget.extend({ selector: ".oe_form_locker", events: { "change select[name='state_id']": "OnChangeStateId", 'submit': '_onSubmit', }, OnChangeStateId: function (e) { var $StateId = $(e.currentTarget); var $CityId = $StateId.closest(".oe_form_locker").find("select[name='city_id']"); $CityId.html(''); var $opt1 = $('<option selected="selected">Ciudad...</option>'); $opt1.appendTo($CityId); ajax.jsonRpc('/ajax/get/country_info', 'call', { 'model': 'res.country.state', 'obj_id': parseInt($StateId.val()) }).then(function (data) { _.each(data["response"], function (item) { var $dynamic_opt = $('<option value="' + item[0] + '"' + ">" + item[1] + '</option>'); $dynamic_opt.appendTo($CityId); }); return; }); }, _onSubmit: function () { var $btn = this.$('button[type="submit"]'); $btn.attr('disabled', 'disabled'); $btn.prepend('<i class="fa fa-refresh fa-spin"/> '); }, }); publicWidget.registry.AddUserTree = publicWidget.Widget.extend({ selector: ".oe_user_tree", events: { "click a.oe_a_delete": "OnClickDeleteRecord", }, OnClickDeleteRecord: function (e) { var $aDelete = $(e.currentTarget); ajax.jsonRpc('/ajax/partner/inactive', 'call', { 'partner_id': parseInt($aDelete.data('id')) }).then(function (data) { window.location.reload(); }); }, }); publicWidget.registry.PackageRecordTree = publicWidget.Widget.extend({
        selector: ".form-add-record", events: { "click a.oe_add_item": "OnClickAddRecord", "click a.oe_item_delete": "OnClickDeleteRecord", "click a.btn-validate": "OnClickValidate", "click a.oe_item_create_record": "OnClickCreateRecord", 'click button[type="submit"]': '_onSubmit', 'click button.oe-submit-product': '_onSubmitValidate', "change input[name='quantity'], input[name='price_unit']": "OnChangeQuantity", "change .category-data": "OnChangeDatalist", }, OnChangeDatalist: function (e) {
            var $dataList = $(e.currentTarget); var $td = $dataList.parent(); if ($td.find("#category_id_list option[value='" + $dataList.val() + "']").attr('data-id') == undefined) { Dialog.alert(this, "La categoría no existe, seleccione una del listado."); $dataList.val(''); return }
            return;
        }, OnClickCreateRecord: function (e) {
            var $tr = $(e.currentTarget).closest('tr'); var $table = $tr.closest('table'); var package_id = $table.data('id') || false; if (package_id === false) { Dialog.alert(this, "Ocurrió algún error, por favor recargue la página."); return }
            var values = {}
            values['package_id'] = parseInt(package_id); values['category_id'] = parseInt($tr.find("select[name='category_id']").val() || false); values['quantity'] = parseInt($tr.find("input[name='quantity']").val() || 1); values['product'] = $tr.find("input[name='product']").val() || false; values['price_unit'] = $tr.find("input[name='price_unit']").val() || false; if (values.product === false || values.price_unit === false) { Dialog.alert(this, "Por favor verifique que tenga establecido el producto y el precio unitario."); return; }
            values['price_unit'] = parseFloat(values['price_unit']); ajax.jsonRpc('/website/item/create', 'call', { 'values': values }).then(function (data) {
                if (data.error === true) { Dialog.alert(this, error.message); return; }
                window.location.reload(); return;
            });
        }, OnClickDeleteRecord: function (e) { var $tr = $(e.currentTarget).closest('tr'); $tr.remove(); this._onUpdateTotal(); }, OnClickAddRecord: function (e) { var $aAddRecord = $(e.currentTarget); var $form = $aAddRecord.closest('form'); var $tbody = $form.find('.oe_item_body'); ajax.jsonRpc('/view/item/render', 'call', {}).then(function (data) { var $tr = $(data['view']); $tr.appendTo($tbody); }); }, OnClickValidate: function (e) { e.preventDefault(); var $btn = $(e.currentTarget); ajax.jsonRpc($btn.attr('href'), 'call', {}).then(function (data) { window.location = '/my/locker/packages'; }); }, OnChangeQuantity: function (e) {
            var self = this; var $tr = $(e.currentTarget).closest('tr'); var line_id = $tr.data('id') || false; var $inputQty = $tr.find('input[name=quantity]'); var input_Qty = parseFloat($inputQty.val() || 1).toFixed(2); var $inputPriceUnit = $tr.find('input[name=price_unit]'); var priceUnit = parseFloat($inputPriceUnit.val() || 0).toFixed(2); var $inputPriceSubtotal = $tr.find('input[name=price_subtotal]'); var subtotal = input_Qty * priceUnit; if (subtotal <= 1) { Dialog.alert(this, "El valor del producto debe ser mayor a $1.00"); return; }
            $inputPriceSubtotal.val(subtotal); if (line_id != false) { ajax.jsonRpc('/ajax/line/update', 'call', { 'line_id': line_id, 'quantity': input_Qty, 'price_unit': priceUnit }).then(function (data) { self._onUpdateTotal(); }); }
        }, _onUpdateTotal: function () {
            var $totalAmount = this.$('input[name=total_amount]'); var $tr = this.$('tbody tr'); var total = 0.0
            $tr.each(function () { var $InputSubtotal = $(this).find("input[name='price_subtotal']"); total = total + parseFloat($InputSubtotal.val() || 0); }); $totalAmount.val(total.toFixed(2));
        }, _onSubmit: function (e) {
            e.preventDefault(); var params = []; var error = false; var error_on = ''; var $submitButton = $(e.currentTarget); $submitButton.attr('disabled', 'disabled'); $submitButton.prepend('<i class="fa fa-refresh fa-spin"/> '); var $form = $submitButton.closest('form'); var $tr = $form.find('tbody tr'); var price_error = false; var total_fob = 0.0; if ($tr.length < 1) { Dialog.alert(this, "Por favor registre sus items, gracias."); return; }
            $tr.each(function () {
                var product = $(this).find("input[name='product']"); var quantity = $(this).find("input[name='quantity']"); var $dataList = $(this).find("input[name='category_id']"); var category_id = $(this).find("#category_id_list option[value='" + $dataList.val() + "']").attr('data-id'); if (category_id == undefined) { error = true; error_on = $dataList.val(); return; }
                var price_unit = $(this).find("input[name='price_unit']"); var shop_name = $(this).find("input[name='shop_name']"); if (parseFloat(price_unit.val()) < 0.05) { price_error = true; return; }
                var qty = parseFloat(quantity.val() || 0); var price = parseFloat(price_unit.val() || 0); var subtotal = qty * price; total_fob += subtotal; params.push({ 'product': product.val(), 'quantity': quantity.val(), 'price_unit': price_unit.val(), 'category_id': category_id, 'shop_name': shop_name.val() })
            }); if (price_error === true) { Dialog.alert(this, "El precio unitario de los productos debe ser mayor o igual a 0.05"); $submitButton.removeAttr('disabled'); $submitButton.find('i').remove(); return; }
            if (total_fob <= 1) { Dialog.alert(this, "El valor total FOB debe ser mayor a $1.00"); $submitButton.removeAttr('disabled'); $submitButton.find('i').remove(); return; }
            if (error === true) { Dialog.alert(this, "La categoría " + error_on + " no existe, seleccione una del listado. Borre el contenido y se desplegará."); $submitButton.removeAttr('disabled'); $submitButton.find('i').remove(); return; }
            this._submitCode(params, $form.attr('action'), $submitButton);
        }, _onSubmitValidate: function (e) {
            e.preventDefault(); var $btn = $(e.currentTarget); $btn.attr('disabled', 'disabled'); $btn.prepend('<i class="fa fa-refresh fa-spin"/> '); var $file = this.$('.oe-upload'); if ($file.val() === "") { Dialog.alert(this, "Por favor adjunte su factura de compra."); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
            ajax.jsonRpc('/package/btn/validate/' + $btn.data('id'), 'call', {}).then(function (data) {
                if (data.success === false) { Dialog.alert(this, "Por favor revise los precios unitarios de sus productos."); $btn.removeAttr('disabled'); $btn.find('i').remove(); return; }
                window.location.href = "/my/locker/packages"; return;
            });
        }, _submitCode: function (params, url, btn) {
            btn.removeAttr('disabled'); btn.find('i').remove(); this._rpc({ route: url, params: { 'items': params } }).then(function (response) {
                if (response.success === false) { Dialog.alert(this, response.message); return; }
                window.location = '/my/locker/packages'; return;
            });
        },
    }); publicWidget.registry.UploadFile = publicWidget.Widget.extend({
        selector: ".oe-upload-file", events: { "change .oe-upload": "OnChangeUploadInput", }, OnChangeUploadInput: function (e) {
            var $this = this; var $inputFile = $(e.currentTarget); var model = $inputFile.data('model'); var binary_field = $inputFile.data('binary'); var binary_field_name = $inputFile.data('name'); var res_id = $inputFile.data('id'); var file = e.target.files[0]; var name = file.name; if (file.type === 'application/pdf' || file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'application/msword' || file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') { var ArrayReader = new FileReader(); ArrayReader.readAsDataURL(file); ArrayReader.onload = function (upload) { var buffer = upload.target.result; buffer = buffer.split(',')[1]; var values = {}; values[binary_field] = buffer; values[binary_field_name] = name; ajax.jsonRpc('/web/custom/update', 'call', { model: model, model_id: res_id, values: values, }).then(function () { var $spanName = $this.$el.closest('ul').find('span.oe-span-name'); $spanName.text(name); Dialog.alert(this, "Archivo cargado correctamente."); }); }; }
            else { $inputFile.val(''); Dialog.alert(this, "Solo puede adjuntar pdf, documentos o fotos."); return; }
        },
    });
});;

/* /theme_e_box/static/src/js/whatsapp.js defined in bundle 'web.assets_frontend_lazy' */
$(function () { $('#WAButton').floatingWhatsApp({ phone: '593993564168', popupMessage: 'Hola,¿cómo podemos ayudarte?', message: "Escribe aquí tu mensaje...", showPopup: true, showOnIE: false, headerTitle: '¡Bienvenid@!', headerColor: '#075e54', backgroundColor: '#075e54'}); }); (function ($) {
    $.fn.floatingWhatsApp = function (options) {
        var settings = $.extend({ phone: '', message: '', size: '50px', backgroundColor: '#25D366', position: 'right', popupMessage: '', showPopup: false, showOnIE: true, autoOpenTimeout: 0, headerColor: '#128C7E', headerTitle: 'WhatsApp Chat', zIndex: 0, buttonImage: '<?xml version="1.0" encoding="UTF-8" standalone="no"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="isolation:isolate" viewBox="0 0 800 800" width="800" height="800"><defs><clipPath id="_clipPath_A3g8G5hPEGG2L0B6hFCxamU4cc8rfqzQ"><rect width="800" height="800"/></clipPath></defs><g clip-path="url(#_clipPath_A3g8G5hPEGG2L0B6hFCxamU4cc8rfqzQ)"><g><path d=" M 787.59 800 L 12.41 800 C 5.556 800 0 793.332 0 785.108 L 0 14.892 C 0 6.667 5.556 0 12.41 0 L 787.59 0 C 794.444 0 800 6.667 800 14.892 L 800 785.108 C 800 793.332 794.444 800 787.59 800 Z " fill="rgb(37,211,102)"/></g><g><path d=" M 508.558 450.429 C 502.67 447.483 473.723 433.24 468.325 431.273 C 462.929 429.308 459.003 428.328 455.078 434.22 C 451.153 440.114 439.869 453.377 436.434 457.307 C 433 461.236 429.565 461.729 423.677 458.78 C 417.79 455.834 398.818 449.617 376.328 429.556 C 358.825 413.943 347.008 394.663 343.574 388.768 C 340.139 382.873 343.207 379.687 346.155 376.752 C 348.804 374.113 352.044 369.874 354.987 366.436 C 357.931 362.999 358.912 360.541 360.875 356.614 C 362.837 352.683 361.857 349.246 360.383 346.299 C 358.912 343.352 347.136 314.369 342.231 302.579 C 337.451 291.099 332.597 292.654 328.983 292.472 C 325.552 292.301 321.622 292.265 317.698 292.265 C 313.773 292.265 307.394 293.739 301.996 299.632 C 296.6 305.527 281.389 319.772 281.389 348.752 C 281.389 377.735 302.487 405.731 305.431 409.661 C 308.376 413.592 346.949 473.062 406.015 498.566 C 420.062 504.634 431.03 508.256 439.581 510.969 C 453.685 515.451 466.521 514.818 476.666 513.302 C 487.978 511.613 511.502 499.06 516.409 485.307 C 521.315 471.55 521.315 459.762 519.842 457.307 C 518.371 454.851 514.446 453.377 508.558 450.429 Z  M 401.126 597.117 L 401.047 597.117 C 365.902 597.104 331.431 587.661 301.36 569.817 L 294.208 565.572 L 220.08 585.017 L 239.866 512.743 L 235.21 505.332 C 215.604 474.149 205.248 438.108 205.264 401.1 C 205.307 293.113 293.17 205.257 401.204 205.257 C 453.518 205.275 502.693 225.674 539.673 262.696 C 576.651 299.716 597.004 348.925 596.983 401.258 C 596.939 509.254 509.078 597.117 401.126 597.117 Z  M 567.816 234.565 C 523.327 190.024 464.161 165.484 401.124 165.458 C 271.24 165.458 165.529 271.161 165.477 401.085 C 165.46 442.617 176.311 483.154 196.932 518.892 L 163.502 641 L 288.421 608.232 C 322.839 627.005 361.591 636.901 401.03 636.913 L 401.126 636.913 L 401.127 636.913 C 530.998 636.913 636.717 531.2 636.77 401.274 C 636.794 338.309 612.306 279.105 567.816 234.565" fill-rule="evenodd" fill="rgb(255,255,255)"/></g></g></svg>', }, options); var isMobile = mobilecheck(); this.addClass('floating-wpp'); var $button = $(document.createElement('div')); var $buttonImageContainer = $(document.createElement('div')); var $popup = $(document.createElement('div')); var $header = $(document.createElement('div')); var $popupMessage = $(document.createElement('div')); var $btnSend = $(document.createElement('div')); var $inputMessage = $(document.createElement('div')); $buttonImageContainer.addClass('floating-wpp-button-image'); $button.addClass('floating-wpp-button').append($(settings.buttonImage)).css({ 'width': settings.size, 'height': settings.size, 'background-color': settings.backgroundColor }); if (!isInternetExplorer() || settings.showOnIE) { $button.append($buttonImageContainer).appendTo(this); }
        $button.on('click', function () {
            if (isMobile && settings.showPopup) { openPopup(); }
            else { sendWhatsappMessage(); }
        }); if (settings.showPopup) {
            var $textarea = $(document.createElement('textarea')); var $closeBtn = $(document.createElement('strong')); var $sendIcon = $('<?xml version="1.0" encoding="UTF-8" standalone="no"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="isolation:isolate" viewBox="0 0 20 18" width="20" height="18"><defs><clipPath id="_clipPath_fgX00hLzP9PnAfCkGQoSPsYB7aEGkj1G"><rect width="20" height="18"/></clipPath></defs><g clip-path="url(#_clipPath_fgX00hLzP9PnAfCkGQoSPsYB7aEGkj1G)"><path d=" M 0 0 L 0 7.813 L 16 9 L 0 10.188 L 0 18 L 20 9 L 0 0 Z " fill="rgb(46,46,46)"/></g></svg>')
...
odoo.define('web_unsplash.beacon', function (require) {
    'use strict'; var publicWidget = require('web.public.widget'); publicWidget.registry.UnsplashBeacon = publicWidget.Widget.extend({
        selector: '#wrapwrap', start: function () {
            var unsplashImages = _.map(this.$('img[src*="/unsplash/"]'), function (img) { return img.src.split('/unsplash/')[1].split('/')[0]; }); if (unsplashImages.length) {
                this._rpc({ route: '/web_unsplash/get_app_id', }).then(function (appID) {
                    if (!appID) { return; }
...
odoo.define('payment_stripe.payment_form', function (require) {
    "use strict"; var ajax = require('web.ajax'); var core = require('web.core'); var Dialog = require('web.Dialog'); var PaymentForm = require('payment.payment_form'); var qweb = core.qweb; var _t = core._t; ajax.loadXML('/payment_stripe/static/src/xml/stripe_templates.xml', qweb); PaymentForm.include({
        willStart: function () { return this._super.apply(this, arguments).then(function () { return ajax.loadJS("https://js.stripe.com/v3/"); }) }, _createPaymentMethod: function (stripe, formData, card, addPmEvent) { if (addPmEvent) { return this._rpc({ route: '/payment/stripe/s2s/create_setup_intent', params: { 'acquirer_id': formData.acquirer_id } }).then(function (intent_secret) { return stripe.handleCardSetup(intent_secret, card); }); } else { return stripe.createPaymentMethod({ type: 'card', card: card, }); } }, _createStripeToken: function (ev, $checkedRadio, addPmEvent) {
            var self = this; if (ev.type === 'submit') { var button = $(ev.target).find('*[type="submit"]')[0] } else { var button = ev.target; }
...
odoo.define('payment_stripe.processing', function (require) {
    'use strict'; var ajax = require('web.ajax'); var rpc = require('web.rpc')
    var publicWidget = require('web.public.widget'); var PaymentProcessing = publicWidget.registry.PaymentProcessing; return PaymentProcessing.include({
        init: function () { this._super.apply(this, arguments); this._authInProgress = false; }, willStart: function () { return this._super.apply(this, arguments).then(function () { return ajax.loadJS("https://js.stripe.com/v3/"); }) }, _stripeAuthenticate: function (tx) {
            var stripe = Stripe(tx.stripe_publishable_key); return stripe.handleCardPayment(tx.stripe_payment_intent_secret).then(function (result) {
                if (result.error) { return rpc.query({ route: '/payment/stripe/s2s/process_payment_error', params: _.extend({}, { reference: tx.reference, stripe_payment_intent_secret: tx.stripe_payment_intent_secret, error: result.error.message }) }).then(() => Promise.reject({ "message": { "data": { "message": result.error.message } } })); }
                return rpc.query({ route: '/payment/stripe/s2s/process_payment_intent', params: _.extend({}, result.paymentIntent, { reference: tx.reference }), });
            }).then(function () { window.location = '/payment/process'; }).guardedCatch(function () { this._authInProgress = false; });
        }, processPolledData: function (transactions) { this._super.apply(this, arguments); for (var itx = 0; itx < transactions.length; itx++) { var tx = transactions[itx]; if (tx.acquirer_provider === 'stripe' && tx.state === 'pending' && tx.stripe_payment_intent_secret && !this._authInProgress) { this._authInProgress = true; this._stripeAuthenticate(tx); } } },
    });
});